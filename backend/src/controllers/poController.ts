...
export const getPOById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!po) { res.status(404).json({ message: 'Not found' }); return; }
    res.json(po);
  } catch (error) {
...
export const confirmPO = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const id = req.params.id as string;
    const userId = req.user!.id;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!po || po.status !== POStatus.DRAFT) throw new Error('Invalid PO state');
...
export const cancelPO = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const id = req.params.id as string;
    const userId = req.user!.id;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!po || po.status === POStatus.CANCELLED) throw new Error('Invalid PO state');
